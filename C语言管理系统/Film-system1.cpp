...
void savedata(struct Film *film)//保存信息到文件中 
{
	FILE *fp;
	struct Film *p;
...
int main()
{
	...
	char tcode[13];
	int i;
	char ccode[13],bcode[13],acode[13];
	char x;
	...
	fp=fopen("managercode.txt","r+");
	fgets(tcode,13,fp);
	fclose(fp);
	if(strcmp(mcode,tcode) == 0)
	{
		printf("\n\n\n\n\n\t\t\t\t\t\t\t是否修改密码（y/n）:");
		scanf("%c",&x);
		if(x=='y'|| x=='Y')
		{
			...
			if(strcmp(tcode,ccode)==0)
			{
				...
				if(strcmp(bcode,acode)==0)
				{
					system("cls");
					printf("\n\n\n\n\n\t\t\t\t\t\t\t修改密码成功！"); 
					Sleep(3000);
					fp=fopen("managercode.txt","w");
					fprintf(fp,"%s",bcode);
					fclose(fp);
